class ArmadaShips
  ...
  def self.perform(start_planet_id, objective_planet_id, num)
    ...
    if MyShip.count < (2000 - num)
      ...
      if ships = MyShip.create_ships_at(num, closest_planet_to_objective, 'armada', prospecting, defense, attack, engineering, 'MINE', planet_to_conquer.id, max_speed, max_fuel, 0, [20, 0, 0, 0])
        ships.each do |ship|
